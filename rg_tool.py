...
def build_app(app, target, is_release=False):
    ...
    # Per-project sdkconfig.defaults overrides target sdkconfig
    app_defaults = os.path.join(os.getcwd(), app, "sdkconfig.defaults")
    if os.path.exists(app_defaults):
        os.environ["SDKCONFIG_DEFAULTS"] = TARGET_SDKCONFIG + ";" + os.path.abspath(app_defaults)
    elif "SDKCONFIG_DEFAULTS" in os.environ:
        os.environ["SDKCONFIG_DEFAULTS"] = TARGET_SDKCONFIG
    args = [IDF_PY, "app",
...
if os.path.exists(TARGET_SDKCONFIG):
    os.environ["SDKCONFIG_DEFAULTS"] = TARGET_SDKCONFIG
