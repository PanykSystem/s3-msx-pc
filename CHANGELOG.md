...
### Code Size Cleanup — Launcher
- **Removed:** `updater.c` / `updater.h` — unused OTA updater module.
- **Removed:** `gui_draw_tab_indicator()` — dead GUI function.
- **Removed:** `prebuild_cache_cb()` — unused prebuild cache callback.
- **Removed:** Odroid Go legacy migration code.
- **Removed:** `BOOK_TYPE_CUSTOM1` / `BOOK_TYPE_CUSTOM2` — unused SDMMC config constants.

### Build System
- **rg_tool.py:** Added per-project `sdkconfig.defaults` support — target defaults + app-specific defaults separated by `;`.
- **Launcher:** Switched to `-O2` (PERF) via `launcher/sdkconfig.defaults`.
- **fmsx:** Remains at `-Og` (DEBUG) with per-file `-O2`/`-O3` overrides. `-Os` + LTO rejected by linker (orphan `.dram0.data` sections, `xt_debugexception` relocation overflow).
